...
public class DataCubeBrowser extends AbstractWidget<DataCubeBrowser.Config> {
	...
	private void populateCentralContainer() {
		...
		List<String> cubeSliceTypes = CubeSPARQL.getType(cubeSliceURI, cubeSliceGraph,
				SPARQL_service);
		...
		if (isCube || isSlice) {
			...
			if (isCube) {
				...
				cubeDimensions = CubeSPARQL.getDataCubeDimensions(cubeSliceURI,
								cubeGraph, cubeDSDGraph, selectedLanguage,defaultLang,ignoreLang,
								SPARQL_service);
				...
				Thread measuresThread= new Thread(new  Runnable() {
					public void run() {
						...
						selectedMeasures=CubeHandlingUtils.getSelectedMeasure(cubeMeasures,
								selectedMeasures);
					}
				});
				
				//Thread to get cube dimensions
				Thread aggregationSetDimsThread= new Thread(new  Runnable() {
					public void run() {
				...
				Thread cubeDimensionsThread= new Thread(new  Runnable() {
					public void run() {
						// Get all Cube dimensions
						cubeDimensions = CubeSPARQL.getDataCubeDimensions(cubeSliceURI,
									cubeGraph, cubeDSDGraph, selectedLanguage,defaultLang,ignoreLang,
				...
				Thread sliceFixedDimensionsValuesThread= new Thread(new  Runnable() {
					public void run() {
				...
				Thread allDimensionsValuesThread= new Thread(new  Runnable() {
					public void run() {
						// Get values for each slice dimension
						allDimensionsValues = CubeHandlingUtils.getDimsValuesFromSlice(
								cubeDimensions, cubeSliceURI, cubeGraph, cubeDSDGraph,
								sliceGraph, useCodeLists, selectedLanguage,defaultLang,ignoreLang,SPARQL_service);
					}
				});
			...
			FCheckBox measureCheckBox = new FCheckBox(
						"measure_" + measureCount, 
						"<font color=\""+measureColors[colorIndex]+"\">"
			...
			// Add top container with available dimensions to browse only if a cube URi is provided
			if (isCube) {
			...
			if(visualDimensions.size()==0){
				...
				if(!cubeDimensions.contains(visualDimensions.get(0))){
					if(visualDimensions.get(1).equals(cubeDimensions.get(0))){
		...
		//		leftcontainer.addStyle("vertical-align", "middle ");
								...
								if (d1Selected.get(0).equals(d2Selected.get(0))) {
									...
									for (Pair<String, Object> pair : d1choices) {
										if (!pair.snd.toString().equals(
											...
											((FComboBox) fc)
													.setPreSelected(pair.snd.toString());
											((FComboBox) fc).populateView();
											break;
										}
									}
			...
			if(!sliceFixedDimensionsValues.isEmpty()||fixedDimensions.size() > 0 ){
			...
			if (!sliceFixedDimensionsValues.isEmpty()) {
				...
				for (LDResource sliceFixedDim : sliceFixedDimensionsValues.keySet()) {
					...
					FLabel fDimLabel = new FLabel("sliceFixedName_" + sliceFixedValues, "<u>"
					...
					if(fDimValue.getURIorLabel().length()>60){
						fDimValueLabel = new FLabel("sliceFixedValue_" + sliceFixedValues, fDimValue.getURIorLabel().substring(0,60)+"...");
					}else{
						fDimValueLabel = new FLabel("sliceFixedValue_" + sliceFixedValues, fDimValue.getURIorLabel());
					}
	...
	private void addFixedDimensions() {
		...
		for (LDResource fDim : fixedDimensions) {
			...
			// Add the label for the fixed cube dimension
			FLabel fDimLabel = new FLabel("fixedDimLabel_"+fixedDims,
					"<u>" + fDim.getURIorLabel() + ":</u>");
			...
			for (LDResource ldr : allDimensionsValues.get(fDim)) {
				...
				if(ldr.getURIorLabel().length()>60){
					fDimCombo.addChoice(ldr.getURIorLabel().substring(0,60)+"...", ldr.getURI());
				}else{
		...
		try {
			...
			while (res.hasNext()) {
				...
				for(LDResource meas:selectedMeasures){
				 measure +="<font color=\""+measureColors[cubeMeasures.indexOf(meas)]+"\">"+
						 "<p align=\"right\">"+bindingSet.getValue("measure"+i).stringValue()+" </p></font> ";
					i++;
				}	
